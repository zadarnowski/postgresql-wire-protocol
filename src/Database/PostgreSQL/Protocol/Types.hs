...
{-
  ...
  -- * Array Fields
  -- | For compactness, we depict those as Haskell arrays (unboxed when
  -- possible) with a 16-bit index type. PostgreSQL documentation is not clear
  -- on whether array sizes should be interpreted as a signed or unsigned
  -- number by the server), but fortunately this doesn't really matter in
  -- practice, since other implementation considerations restrict the result
  -- sets to far smaller number of columns. The official upper bound is 1600,
  -- although current implementations are capable of accommodating up to 1736
  -- columns in query results. In our implementation, we treat array indexes as
  -- signed but reject all arrays with sizes greater than 32767 as invalid.
  Array16, UArray16,
  -- * Message Types
  SessionMessage (..),
   currentMajorVersion,
