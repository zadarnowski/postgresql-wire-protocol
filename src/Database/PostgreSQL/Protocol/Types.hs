...
data SessionMessage =
  ...
  -- configured at runtime using the SQL @SET@ command, 'STARTUP_MESSAGE'
  ...
  -- backend process with the process ID @pid@. In order to demonstrate
  -- authority to interact with this backend process, the frontend must include
  -- in the message the unique 32-bit key @secret@ generated by the backend
  -- process and supplied to the frontend in a 'BACKEND_KEY_DATA' message sent
  -- as part of the session establishment protocol of the targeted
  -- communication session.
  CANCEL_REQUEST !ProcessID !Word32 |
...
-- | The type of a special-case response to an 'SSL_REQUEST' message described
-- above.

data SSLResponse =

  -- | Indicates to the frontend that the backend has accepted the
  -- 'SSL_REQUEST'. The frontend should perform a standard SSL startup handshake
  ...
  -- channel.
  SSL_REQUEST_ACCEPTED |

  -- | Indicates to the frontend that the backend has rejected the
  -- 'SSL_REQUEST'. The frontend has an option of abandoning the connection by
  -- closing the underlying socket, or proceeding with an unencrypted session
  -- by sending the usual 'STARTUP_MESSAGE' or 'CANCEL_REQUEST' over the same
  -- socket without encryption.
  SSL_REQUEST_REJECTED |
  ...
  -- and likely do not exist in the wild any more. In this case the connection
  ...
  SSL_REQUEST_FAILED NoticeFields
...
-- documentation, with exception of the 'STARTUP_MESSAGE', 'CANCEL_REQUEST'
-- and 'SSL_REQUEST' message types that are defined separately as
-- 'SessionMessage' values.
...
data FrontendMessage =
  ...
  -- back to the frontend in the 'COPY_IN_RESPONSE', 'COPY_OUT_RESPONSE' or
  ...
  COPY_IN_DATA LazyByteString |

  -- | Sent after the final 'COPY_IN_DATA' message of a given @COPY@
  -- subprotocol session, indicates successful completion of an entire @COPY@
  -- data stream.
  COPY_IN_DONE |
  ...
  -- execution of a PostgreSQL function with the given object ID @oid@,
  -- supplying it an array of argument values @avs@ encoded in the transfer
  -- format specified by the array @afs@, and expecting the function's sole
  -- result value to be encoded using the transfer format @rf@. As for 'BIND'
  -- messages, @afs@ can be an empty array if all argument values are supplied
  ...
  -- the SQL command @q@.  The statement name can be set to 'ByteString.null'
  -- to create the default unnamed statement. The array @pts@ specifies object
  -- IDs of PostgreSQL types for any query parameters appearing in @q@. It is
  -- not required to specify types for all query parameters and may even be
  -- left empty if not required; the types of any parameters omitted from @pts@
  -- are then inferred directly from the query string @q@ itself.
...
data BackendMessage =
  ...
  -- frontend to issue query cancellation requests (see: 'CANCEL_REQUEST'
  -- message type above.)
  BACKEND_KEY_DATA !ProcessID !Word32 |
  ...
  -- | A message of the form “@COMMAND_COMPLETE t@” is sent by the backend to
  ...
  -- 'DATA_ROW' messages. The /result tag/ @t is usually a terse but
  ...
  --   * For @DELETE@ commands, the tag has the form “@DELETE /n/@”,
  --     where @/n/@ is the number of rows deleted.
  --   * For @UPDATE@ commands, the tag has the form “@UPDATE /n/@”,
  ...
  --   * For @MOVE@ commands, the tag has the form “@MOVE /n/@”,
  --     where @/n/@ is the number of rows by which the cursor's
  --     position has been changed.
  --   * For @FETCH@ commands, the tag has the form “@FETCH /n/@”,
  --     where @/n/@ is the number of rows that have been retrieved
  --     from the cursor.
  --   * For @COPY@ commands, the tag has the form “@COPY /n/@”,
  ...
  -- 'COPY_OUT_DATA' message, but frontends are allowed to divide stream data
  -- into chunks arbitrarily without regard of data row boundaries.
  COPY_OUT_DATA LazyByteString |

  -- | Sent after the final 'COPY_OUT_DATA' message of a given @COPY@
  -- subprotocol session, indicates successful completion of an entire @COPY@
  -- data stream.
  COPY_OUT_DONE |
  ...
  -- by a 'COPY_IN_DONE', or, if it is not prepared to do so, send a
  -- 'COPY_FAIL' message back to the server.
  --
  ...
  -- 'TEXT_STREAM_FORMAT'.
  COPY_IN_RESPONSE !StreamFormat [Format] |
  ...
  -- should be followed immediately by zero or more 'COPY_OUT_DATA' messages
  -- and completed with 'COPY_OUT_DONE'.
  --
  -- The /stream format/ parameter @f@ defines the overall format of the data
  -- stream requested by the backend, while the array @fs@ defines the transfer
  -- formats of the individual data fields in each row, and must always be set
  -- to 'TEXT_FORMAT' if the overall format of the stream @f@ is set to
  -- 'TEXT_STREAM_FORMAT'.
  COPY_OUT_RESPONSE !StreamFormat [Format] |
  ...
  -- to 'TEXT_FORMAT' if the overall format of the stream @f@ is set to
  -- 'TEXT_STREAM_FORMAT'.
  COPY_BOTH_RESPONSE !StreamFormat [Format] |
  ...
  -- messages from the backend at any time after the initial 'STARTUP_MESSAGE'
  -- of a communication session.
  NOTICE_RESPONSE NoticeFields |
  ...
  NOTIFICATION_RESPONSE !ProcessID ByteString LazyByteString |
  ...
  PARAMETER_DESCRIPTION [ObjectID] |
  ...
  PARAMETER_STATUS ByteString LazyByteString |
  ...
  ROW_DESCRIPTION [FieldDescription]

  deriving (Eq, Ord, Read, Show)


-- | Details of a backend response to a frontend's authentication request
-- depicted by a session's 'STARTUP_MESSAGE'.
data AuthenticationResponse =
  ...
  -- The frontend should respond with a 'PASSWORD_MESSAGE' containing an
  ...
  -- 32-bit /salt/ @s@.  The frontend should respond with a
  -- “'PASSWORD_MESSAGE x'”, in which @x@ is a byte string derived from the
  -- user's login name @u@, password @p@ and the supplied salt @ss@ as
  -- follows:
  --
  -- @
  --      "md5" <> md5 (md5 (/p/ <> /u/) <> /ss/
  ...
  -- the server responds with an 'AUTHENTICATION_OK', otherwise it responds
  -- with an 'ERROR_RESPONSE'. This message type is only issued by versions of
  ...
  -- 'PASSWORD_MESSAGE' with the first part of the GSSAPI data stream. If
  ...
  -- SSPI negotiation, i.e., an 'AUTHENTICATION_GSS', 'AUTHENTICATION_SSPI' or
  -- an earlier 'AUTHENTICATION_GSS_CONTINUE' message. If the GSSAPI or SSPI
  -- data in this message indicates more data is needed to complete the
  -- authentication, the frontend must send that data as another
  -- 'PASSWORD_MESSAGE'.  If GSSAPI or SSPI authentication is completed by this
  -- message, the server will eventually send 'AUTHENTICATION_OK' to indicate
  ...
  -- user.
  AUTHENTICATION_MISCELLANEOUS !Word32 LazyByteString
...
-- | A data type used to describe a single result field in a 'ROW_DESCRIPTION'
-- message.
data FieldDescription = FIELD_DESCRIPTION {
  ...
  -- ^ The format code used by the field; in a 'ROW_DESCRIPTION' message
...
-- using the @COPY@ subprotocol. At the moment, PostgreSQL defines two: the
...
-- | Transaction status codes returned in 'READY_FOR_QUERY' messages.
...
-- problem.  may contain newline characters, which should, however, be
