...
--
-- In PostgreSQL /object identifiers/ or /OIDs/ are used to refer to most
-- database objects, including runtime concepts such as tables, functions,
-- types, cursors and even transactions. Most of these OIDs are allocated
-- dynamically at runtime and must be queried from system tables, but a handful
-- has been hard-coded in the PostgreSQL source code to enable streamlined
-- access to built-in types and other useful concepts. For example, the use of
-- these built-in OIDs makes it easy to request binary encoding of results from
-- queries returning standard SQL and PostgreSQL types such as @INTEGER@ and
-- @TEXT@ without resorting to expensive system catalogue queries.
--
...
--
-- For conciseness and predictability, we strive to give these 'ObjectID'
-- patterns uppercase names identical to those used in the PostgreSQL query
-- language, selecting the unabbreviated standard SQL spelling of type names
-- whenever available, e.g., @BIGINT@ or @CHARACTER@. For brevity, we drop the
-- @OID@ suffix features in the C constants from @pg_type.h@.
--
...
--
-- In the following documentation, OID constants are listed in the order of
-- their introduction in @pg_type.h@, which mostly follows their numeric values
-- and generally bears little relation to their semantic meanings.
--
...
-- “types”, such as @SMGR@ which is described in the source code as “storage
-- manager”, but are nevertheless included in the @pg_type@ catalogue (probably
-- for bootstrapping purposes) and hence can be used as column types in actual
-- database tables. You can even create a column with an @UNKNOWN@ type! For
-- completeness, all of these constants are included below, but it's probably
-- best to refrain from their use in real programs.
...
-- | Null object ID reserved to represent lack of explicit object ID
--   information in PostgreSQL wire protocol exchanges.
...
-- | Well-known object ID of PostgreSQL array type
--   @CHARACTER(/n/)[]@ or @BPCHAR(/n/)[]@.
...
-- | Well-known object ID of PostgreSQL type @ACLITEM@
--   used to represent access control lists.
pattern ACLITEM :: ObjectID
pattern ACLITEM = 1033

-- | Well-known object ID of PostgreSQL array type @ACLITEM[]@
--   used to represent arrays of access control lists.
...
-- | Well-known object ID of the standard SQL type
--   @CHARACTER(/n/)@ or @BPCHAR(/n/)@.
...
-- | Well-known object ID of PostgreSQL type
...
-- | Well-known object ID of PostgreSQL type
--   @BIT(/n/)@ used to represent fixed-length bit strings.
...
-- | Well-known object ID of PostgreSQL type @REFCURSOR@
--   used to represent references to cursors or portal names.
pattern REFCURSOR :: ObjectID
pattern REFCURSOR = 1790

-- | Well-known object ID of PostgreSQL array type @REFCURSOR[]@
--   used to represent arrays of references to cursors or portal names.
pattern REFCURSOR_ARRAY :: ObjectID
pattern REFCURSOR_ARRAY = 2201

-- | Well-known object ID of PostgreSQL type @REGPROCEDURE@
--   used to represent registered procedures with arguments.
pattern REGPROCEDURE :: ObjectID
pattern REGPROCEDURE = 2202

-- | Well-known object ID of PostgreSQL type @REGOPER@
--   used to represent registered operators.
pattern REGOPER :: ObjectID
pattern REGOPER = 2203

-- | Well-known object ID of PostgreSQL type @REGOPERATOR@
--   used to represent registered operators with arguments.
pattern REGOPERATOR :: ObjectID
pattern REGOPERATOR = 2204

-- | Well-known object ID of PostgreSQL type @REGCLASS@
--   used to represent registered classes.
pattern REGCLASS :: ObjectID
pattern REGCLASS = 2205

-- | Well-known object ID of PostgreSQL type @REGTYPE@
--   used to represent registered types.
pattern REGTYPE :: ObjectID
pattern REGTYPE = 2206

-- | Well-known object ID of PostgreSQL array type @REGPROCEDURE[]@ used to
--   represent arrays of registered procedures with arguments.
pattern REGPROCEDURE_ARRAY :: ObjectID
pattern REGPROCEDURE_ARRAY = 2207

-- | Well-known object ID of PostgreSQL array type @REGOPER[]@
--   used to represent arrays of registered operators.
pattern REGOPER_ARRAY :: ObjectID
pattern REGOPER_ARRAY = 2208

-- | Well-known object ID of PostgreSQL array type @REGOPERATOR[]@
--   used to represent arrays of registered operators with arguments.
pattern REGOPERATOR_ARRAY :: ObjectID
pattern REGOPERATOR_ARRAY = 2209

-- | Well-known object ID of PostgreSQL array type @REGCLASS[]@
--   used to represent arrays of registered classes.
pattern REGCLASS_ARRAY :: ObjectID
pattern REGCLASS_ARRAY = 2210

-- | Well-known object ID of PostgreSQL array type @REGTYPE[]@
--   used to represent arrays of registered types.
